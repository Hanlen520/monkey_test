...
if aw == 'y':
    ...
    for i in range(n):
        ...
        phone_model =os.popen("adb  -s "+ dev + " -d shell getprop ro.product.model")	#model = phone_model[0][17:].strip('\r\n')
        model=phone_model
        #print(model)
        #phone_name = os.popen("adb -s " + dev + ' shell cat /system/build.prop | find "ro.product.brand="').readlines()  # 获取手机名称
        phone_name = os.popen("adb  -s "+ dev + " -d shell getprop ro.product.brand")
        #roname = phone_name[0]
        name = phone_name
