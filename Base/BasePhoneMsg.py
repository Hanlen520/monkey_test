...
def getModel(dev):
    ...
    #使用另一种方式获取信息，防止出现cat /system/build.prop无权限的情况（华为手机）
    result["release"] = subprocess.check_output("adb -s "+ dev + " shell getprop ro.build.version.release").decode() #  Android 系统，如anroid 4.0
    result["phone_name"] = subprocess.check_output("adb  -s "+ dev + " -d shell getprop ro.product.model").decode() # 手机名
    result["phone_model"] = subprocess.check_output("adb  -s "+ dev + " -d shell getprop ro.product.brand").decode() # 手机品牌
